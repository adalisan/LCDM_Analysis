...
for (i in 1:length(fileNames)){ distsl[i]<-read.table(fileNames[i]) }
...
if(length(out)!=0){
#echo $results.txt
if(b[1]){cat("p-value for MWU Test: ", out[[1]]$p.value,"\n",file="/cis/home/epostell/Projects/def_scz/pt_pooled/lh_deno.txt")}
if(b[2]){cat("p-value for KS Test Two-sided: ", out[[2]]$p.value,"\n",file="/cis/home/epostell/Projects/def_scz/pt_pooled/lh_deno.txt",append=TRUE)}
...
#if(b[1]){cat("Result for MWU Test: ",unlist(out[[1]]),"\n",file="result.txt")}
#if(b[2]){cat("\nResult for KS Test: ",unlist(out[[2]]),"\n",file="result.txt",append=TRUE)}
#if(b[3]){cat("\nResult for Welch's t-test: ",unlist(out[[3]]),"\n",file="result.txt",append=TRUE)}
#if(b[4]){cat("\nResult for KW Test: ",unlist(out[[4]]),"\n",file="result.txt",append=TRUE)}
#if(b[5]){cat("\nResult for ANOVA F Test: ",unlist(out[[5]]),"\n",file="result.txt",append=TRUE)}
} else { cat("No test was performed",file="result.txt",append=TRUE)}